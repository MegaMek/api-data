...
extension BattleTech.Faction {
    static func findOrNew(
      ...
      on database: Database
        ...
        guard let foundFaction = try await BattleTech.Faction.query(on: database)
    ...
    static func updateParent(
      ...
      guard let faction = try await BattleTech.Faction.query(on: database)
        .filter(\.$factionKey == importableFaction.key)
        .first() else { return }

      let factionKeys = parentFactionKey.split(separator: ",", omittingEmptySubsequences: true)
      for parentKey in factionKeys {
        guard let parentFaction = try await BattleTech.Faction.query(on: database)
