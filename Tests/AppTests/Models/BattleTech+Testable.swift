...
extension BattleTech.Ammo {
  static func create(
    ...
    let munitionType = try await BattleTech.MunitionType.create(
      name: "Test Munition Type", on: database)
    ...
    try await ammo.attachAliases(aliases: ["TestAliase"], on: database)
    return ammo
  }
}
...
extension BattleTech.Equipment {
  static func create(
    ...
    let techLevel = try await BattleTech.TechLevel.create(
      name: "Test Equipment Level", on: database)

    let equipment = BattleTech.Equipment(
      name: name,
      techBase: techBase,
      techRating: techRating,
      techLevelStatic: techLevel,
    ...
    try await equipment.attachRules(rules: ["Test Rule"], on: database)
    try await equipment.attachAliases(aliases: ["TestAliase"], on: database)
...
extension BattleTech.Weapon {
  static func create(
    ...
    try await weapon.attachAliases(aliases: ["TestAliase"], on: database)
